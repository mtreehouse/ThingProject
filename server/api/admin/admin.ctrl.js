...
export const list = async ctx => {
    ...
    await db.promise().query("SELECT id, name, my_phone, his_phone, receipt_id, date_format(created_date,'%Y-%m-%d')created_date, ischecked FROM thing_members ORDER BY id DESC")
        .then(([rows, fields]) => {
                ctx.body = {
                    members: rows,
                    columns: fields
